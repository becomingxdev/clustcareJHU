...
const ViewDoctorsModal = ({ isOpen, onClose, clinic }) => {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/clinic/cluster/clinics/${clinic.id}/doctors`, {
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]">
        <div className="bg-gray-900 px-6 py-4 flex justify-between items-center text-white shrink-0">
          <h3 className="text-lg font-semibold">Doctors at {clinic?.name}</h3>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="py-2 text-sm font-semibold text-gray-600">Specialization</th>
              ...
              <tbody>
                {doctors.map((doc, idx) => (
                  <tr key={idx} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                    <td className="py-3 text-gray-800">{doc.name}</td>
                    <td className="py-3 text-gray-600">{doc.specialization || 'General'}</td>
                    <td className="py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        doc.active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
