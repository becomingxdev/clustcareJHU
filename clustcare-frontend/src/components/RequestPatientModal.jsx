...
const RequestPatientModal = ({ isOpen, onClose, targetClinic, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/api/clinic/cluster/patient-requests', {
        method: 'POST',
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 opacity-100">
        <div className="bg-gray-900 px-6 py-4 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-white">Request Patient Record</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Enter the unique ID of the patient to request their full record.
            </p>
          </div>
...
export default RequestPatientModal;
